...
# ▼▼▼ PATCH 1: ANALYSIS_PROMPT — yangilandi ▼▼▼
...
RULES:
- NEVER include brand names — write "Brand: not applicable"
...
# ▼▼▼ PATCH 2: get_infographic_prompt_system() — yangilandi ▼▼▼
...
BAD (machine translation) → GOOD (natural Uzbek):
- "Naqshli materiallar" → "Terga chidamli mato"
- "Qulay yurish tajribasi" → "Kun bo'yi qulay his"
- "Qo'llab-quvvatlovchi tuzilma" → "Oyoq charchamasdan ishlaydi"
- "Yuqori sifatli mahsulot" → "Yillar davomida saqlanadi"
- "Zamonaviy dizayn" → "Ko'z tortuvchi ko'rinish"
- "Funksional xususiyatlar" → "Kundalik hayotni osonlashtiradi"
- "Ishonchli material" → "Uzoq xizmat qiladi"

NATURAL UZBEK PATTERNS:
- Titles (2-3 words): "Qulay va chidamli", "Har kuni kiy", "Tez quriydi"
- Benefits (5-8 words): "Og'irligini sezmaysiz — yengil", "Suv o'tkazmaydigan himoya"
- Tone: Warm, direct, like a trusted friend recommending something
...
BAD (machine translation) → GOOD (natural Russian):
- "Узорчатый материал" → "Не промокает под дождём"
- "Поддерживающая конструкция" → "Ноги не устают весь день"
- "Удобный опыт ходьбы" → "Комфорт с первого шага"
- "Высококачественный продукт" → "Прослужит не один сезон"
- "Современный дизайн" → "Выглядишь стильно всегда"
- "Функциональные характеристики" → "Всё продумано для вас"
- "Эффективная поддержка" → "Спина скажет спасибо"

NATURAL RUSSIAN PATTERNS:
- Titles (2-3 words): "Носи с удовольствием", "Тепло весь день", "Лёгкий и прочный"
- Benefits (5-8 words): "Не промокнет даже в ливень", "Греет до −30°C"
- Tone: Conversational, punchy, buyer-focused — like top Wildberries listings
...
- Keep SHORT: 2-3 word titles, 5-8 word descriptions

- NO CapsLock

- NO emoji in image text

- Every text element must pass: "Does this sound like a real marketplace seller wrote it?"

Features:

- Show 3-4 feature points with minimal icons on the LEFT side

- Each feature: icon + bold title (2-3 words, customer benefit) + short description below (5-8 words)
...
- Use product type + key feature as headline instead of brand name
...
5. After writing the prompt, review every text element: if any sounds like a literal English translation, rewrite it
...
{"role": "user", "content": (
    ...
    f"IMPORTANT: After writing the prompt, review every text element you included. "
    f"If any text sounds like a literal translation or uses abstract nouns instead of customer benefits, "
    f"rewrite it. Output only the final improved prompt."
)},
...
# ▼▼▼ PATCH 3: write_promo_prompts() system content — yangilandi ▼▼▼
...
# ▼▼▼ PATCH 4: CARD_TEXT_SYSTEM — yangilandi ▼▼▼
CARD_TEXT_SYSTEM = {
...
Matnni O'ZBEKCHA fikrlab yoz. Ingliz tilidan so'zma-so'z tarjima QILMA.

❌ Mashinada tarjima belgilari (TAQIQLANGAN):

- "Yuqori sifatli naqshli materiallardan tayyorlangan"

- "Qulay yurish tajribasini ta'minlaydi"

- "Qo'llab-quvvatlovchi funksional tuzilmali dizayn"

✅ Tabiiy o'zbekcha namunalar:

- "Kundalik kiyim uchun qulay va chidamli"

- "Tez quriydi, yengil, terga chidamli"

- "Oilangiz uchun ishonchli tanlov"

QOIDA: Har bir jumlani yozishdan oldin o'zingizdan so'rang:

"O'zbekiston bozorida professional sotuvchi shunday yozarmi?" — Yo'q bo'lsa, qayta yoz.""",
...
Пиши КАК НОСИТЕЛЬ ЯЗЫКА. Не переводи с английского дословно.

❌ Признаки машинного перевода (ЗАПРЕЩЕНО):

- "Обеспечивает надёжную поддерживающую структуру"

- "Высококачественный опыт комфортной ходьбы"

- "Функциональные узорчатые материалы высокого класса"

✅ Примеры естественного русского стиля:

- "Удобно носить весь день — не жмёт, не натирает"

- "Не промокает, быстро сохнет, выглядит аккуратно"

- "Подходит для повседневной носки и активного отдыха"

ПРАВИЛО: Перед каждой фразой спроси себя:

"Так написал бы реальный продавец на Wildberries или Ozon?" — Нет? Перепиши.""",

}
# ▲▲▲ PATCH 4 end ▲▲▲


# ▼▼▼ PATCH 5: DESCRIPTION_SYSTEM — yangilandi ▼▼▼
DESCRIPTION_SYSTEM = {
...
🚫 Stop-so'zlar taqiqlangan: aksiya, bepul, chegirma, top, xit, eng yaxshi, arzon.

📣 USLUB QOIDASI:

O'zbekcha tavsifni TO'G'RIDAN TO'G'RI o'zbekcha fikrlab yoz.

Inglizcha yoki ruscha iboralarni tarjima qilma — bu matndagi sun'iy his beradi.

❌ TAQIQLANGAN uslub:

- "Mahsulot yuqori sifatli materiallardan tayyorlangan bo'lib, foydalanuvchiga qulay tajriba taqdim etadi."

- "Zamonaviy texnologiyalar yordamida ishlab chiqarilgan ushbu mahsulot..."

✅ TABIIY uslub:

- "Bu mahsulotni kundalik hayotda ishlatish juda oson."

- "Ko'p yillik tajriba shuni ko'rsatadiki, bunday buyumlar oilada uzoq xizmat qiladi."

- "Sovg'a sifatida ham ajoyib tanlov — har kim xursand bo'ladi." """,
...
🚫 Стоп-слова запрещены: акция, бесплатно, скидка, топ, хит, лучший, дёшево.

📣 ПРАВИЛО СТИЛЯ:

Русский текст пиши КАК ЖИВОЙ ЧЕЛОВЕК — продавец, который знает свой товар.

Не копируй канцелярский стиль, не переводи с английского дословно.

❌ ЗАПРЕЩЁННЫЙ стиль:

- "Данное изделие изготовлено из высококачественных материалов, обеспечивающих надёжную эксплуатацию."

- "Современная конструкция обеспечивает оптимальные функциональные характеристики."

✅ ЕСТЕСТВЕННЫЙ стиль:

- "Пользоваться этой вещью удобно каждый день — быстро привыкаешь и уже не представляешь без неё."

- "Отличный подарок на любой случай — упаковка аккуратная, выглядит дорого."

- "Материал приятный на ощупь, не линяет после стирки, держит форму." """,
